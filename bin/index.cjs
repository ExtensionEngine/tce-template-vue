...
async function cloneRepository() {
  ...
  const cloneCommand = shell.exec(
    `git clone --depth 1 https://github.com/ExtensionEngine/tce-template-vue ${projectName}`
  );
...
async function cleanup() {
  ...
  if (
    deleteGitCommand.code !== SUCCESS_CODE ||
    deleteBinCommand.code !== SUCCESS_CODE
  ) {
